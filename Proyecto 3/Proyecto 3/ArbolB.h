...
nodoB* nodoB::search(int buscado) {
	int i = 0;
...
bool nodoB::booleanSearch(int buscado) {
	nodoB* aux = search(buscado);
	if (aux == NULL) {
		return false;
	}
	else
...
void arbolB::Insertar(int ced, string nombre) {
	if (root == NULL) { //si el arbol esta vacio inserta en la raiz
		root = new nodoB(5, true);
		root->claves[0] = ced;
...
void nodoB::insertNonFull(int ced, string nombre) {
	int i = cuentas - 1;
...
void nodoB::splitChild(int i, nodoB* y) {
	nodoB* z = new nodoB(y->tam, y->hoja); //pagina donde se almacenara la mitad de la pagina por dividir
	z->cuentas = tam - 1;
...
nodoB* arbolB::busqueda(int buscado) {
	return (root == NULL) ? NULL : root->search(buscado);
}

bool arbolB::ClaveExiste(int buscado) {
	bool val = root->booleanSearch(buscado);
	return val;
}

void arbolB::ModificarNombre(int ced, string nom) {
	if (root != NULL) {
		root->ModificarN(ced, nom);
	}
}

void nodoB::ModificarN(int ced, string nom) {
	int i = 0;
...
void nodoB::deletion(int buscado) {
...
void nodoB::borrowFromPrev(int idx) {
	nodoB* hijo = ramas[idx];
	nodoB* hermano = ramas[idx - 1];
