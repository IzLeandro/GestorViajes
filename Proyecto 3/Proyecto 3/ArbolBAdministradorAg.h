...
class nodoBAgencia
{
	...
	nodoBAgencia** ramas;   //Ramas
	int cuentas;	 //Cuentas
	bool hoja;
	int tam;
public:

	nodoBAgencia(int t, bool leaf) {
		tam = t;
		claves = new int[1000 * tam - 1];
		nombres = new string[1000 * tam - 1];
		ramas = new nodoBAgencia * [1000 * tam];
		cuentas = 0;
		hoja = leaf;
	}
	void insertNonFull(int k, string a);
	void splitChild(int i, nodoBAgencia* y);
	void traverse();
	nodoBAgencia* search(int k);
...
private:
	nodoBAgencia* root;
public:
	arbolBAgencia() { root = NULL; }
	void CargarArchivos();
	void Mostrar();
	nodoBAgencia* busqueda(int buscado);
...
void arbolBAgencia::CargarArchivos() {
	string cadena;
	string letra;
	string codigo;
	string nombre;

	ifstream archivo("AdministradorA.txt");
...
nodoBAgencia* nodoBAgencia::search(int buscado) {
	int i = 0;
...
bool nodoBAgencia::booleanSearch(int buscado) {
	nodoBAgencia* aux = search(buscado);
	if (aux == NULL) {
		return false;
	}
	else
...
void arbolBAgencia::Insertar(int ced, string nombre) {
	if (root == NULL) { //si el arbol esta vacio inserta en la raiz
		root = new nodoBAgencia(5, true);
		root->claves[0] = ced;
		...
		if (!ClaveExiste(ced)) { //verifica si la identificacion ya existe
			...
			if (root->cuentas == 2 * 5 - 1) { //si la pagina esta llena
				nodoBAgencia* nuevo = new nodoBAgencia(5, false);
				nuevo->ramas[0] = root;
...
void nodoBAgencia::insertNonFull(int ced, string nombre) {
	int i = cuentas - 1;
...
void nodoBAgencia::splitChild(int i, nodoBAgencia* y) {
	nodoBAgencia* z = new nodoBAgencia(y->tam, y->hoja); //pagina donde se almacenara la mitad de la pagina por dividir
	z->cuentas = tam - 1;
...
void arbolBAgencia::Mostrar() {
	if (root != NULL)
		root->traverse();
}

nodoBAgencia* arbolBAgencia::busqueda(int buscado) {
	return (root == NULL) ? NULL : root->search(buscado);
}

bool arbolBAgencia::ClaveExiste(int buscado) {
	bool val = root->booleanSearch(buscado);
	return val;
}

void arbolBAgencia::ModificarNombre(int ced, string nom) {
	if (root != NULL) {
		root->ModificarN(ced, nom);
	}
}

void nodoBAgencia::ModificarN(int ced, string nom) {
	int i = 0;
...
//BORRADO
int nodoBAgencia::findKey(int buscado) {
	int idx = 0;
...
void nodoBAgencia::deletion(int buscado) {
...
void nodoBAgencia::removeFromLeaf(int idx) {
...
void nodoBAgencia::removeFromNonLeaf(int idx) {
	int k = claves[idx];
	if (ramas[idx]->cuentas >= tam) {
...
int nodoBAgencia::getPredecessor(int idx) {
	nodoBAgencia* cur = ramas[idx];
	while (!cur->hoja) {
...
string nodoBAgencia::getNPred(int idx) {
	nodoBAgencia* cur = ramas[idx];
	while (!cur->hoja) {
		cur = cur->ramas[cur->cuentas];
	}
...
int nodoBAgencia::getSuccessor(int idx) {
	nodoBAgencia* cur = ramas[idx + 1];
	while (!cur->hoja) {
		cur = cur->ramas[0];
	}
	return cur->claves[0];
...
string nodoBAgencia::getNSucc(int idx) {
	nodoBAgencia* cur = ramas[idx + 1];
	while (!cur->hoja) {
		cur = cur->ramas[0];
	}
...
void nodoBAgencia::fill(int idx) {
...
void nodoBAgencia::borrowFromPrev(int idx) {
	nodoBAgencia* hijo = ramas[idx];
	nodoBAgencia* hermano = ramas[idx - 1];
...
void nodoBAgencia::borrowFromNext(int idx) {
	nodoBAgencia* hijo = ramas[idx];
	nodoBAgencia* hermano = ramas[idx + 1];
...
void nodoBAgencia::merge(int idx) {
	nodoBAgencia* hijo = ramas[idx];
	nodoBAgencia* hermano = ramas[idx + 1];
...
void arbolBAgencia::Borrar(int k) {
	if (root == NULL) {
